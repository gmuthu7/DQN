...
class Builder:
    ...
    def env(self, env_name: str, num_envs: int):
        self.env: VectorEnv = gym.vector.make(env_name, num_envs)
    ...
    def simple_neural_network(self, num_hidden: int):
        num_inputs = self.env.single_observation_space.shape[0]
        num_outputs = self.env.single_action_space.n
    ...
    def annealed_epsilon(self, end_epsilon: int, anneal_finished_step: int):
        ...
        initial_epsilon = (1. - end_epsilon * min(1., self.initial_no_learn_steps / anneal_finished_step)) / (
                1. - min(1., self.initial_no_learn_steps / anneal_finished_step))
        self.epsilon_scheduler = functools.partial(fn, initial_epsilon, end_epsilon, anneal_finished_step)
        return self

    def epsilon_policy(self):
        action_sampler = lambda: torch.randint(0, self.env.action_space.n, size=(self.env.num_envs,))
        greedy_policy = GreedyPolicy(self.vfa.val)
        self.policy = EpsilonPolicy(self.epsilon_scheduler, action_sampler, greedy_policy)

    def dqn(self, update_freq: int, target_update_freq: int, num_update_steps: int):
        self.agent = Dqn(self.buffer, self.vfa, self.policy, update_freq, target_update_freq,
                         self.initial_no_learn_steps, num_update_steps)

    def double_dqn(self, update_freq: int, target_update_freq: int, num_update_steps: int):
    ...
    def trainer(self, eval_freq: int, eval_num_episodes: int, seed: int):
        self.trainer = Trainer(eval_freq, eval_num_episodes, self.eval_env, Evaluator(), seed, self.logger)

    def build(self) -> Tuple[VectorEnv, Dqn, Trainer]:
        return self.env, self.agent, self.trainer
